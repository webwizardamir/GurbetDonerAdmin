...
export default function PortalCreateForm({
  ...
  const [mode, setMode] = useState<OnboardMode>('password')
  ...
  const tabActive = 'bg-white dark:bg-slate-800 text-green-700 dark:text-green-400 shadow-sm'
  const tabInactive = 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-1 p-1 bg-slate-100 dark:bg-slate-700/50 rounded-xl">
        <button type="button" onClick={() => setMode('password')}
          className={`${tabBase} ${mode === 'password' ? tabActive : tabInactive}`}>
          <KeyRound className="w-4 h-4" />{t('portal.access.mode.password')}
        </button>
        <button type="button" onClick={() => setMode('invite')}
          className={`${tabBase} ${mode === 'invite' ? tabActive : tabInactive}`}>
          <Link2 className="w-4 h-4" />{t('portal.access.mode.invite')}
        </button>
      </div>
      ...
      {mode === 'password' ? (
        ...
        <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl">
          <p className="text-sm text-blue-800 dark:text-blue-300">{t('portal.access.inviteExplain')}</p>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-3 pt-2">
        <button type="submit" disabled={actionLoading}
          ...
          {actionLoading
            ? (<><Loader2 className="w-4 h-4 animate-spin" />{t('portal.access.creating')}</>)
            : mode === 'invite' ? t('portal.access.generateInvite') : t('portal.access.enable')}
        </button>
