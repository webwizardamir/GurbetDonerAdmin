...
import {
  ...
  FileText, Copy, AlertTriangle, RefreshCw,
} from 'lucide-react'
...
export default function DeliveryRoutePanel({ day, dayLabel, city, onClose }: Props) {
  const { t } = useTranslation()
  const r = useDeliveryRoute(day, city)
  ...
  const openInMaps = () => {
    if (!r.route) return
    window.open(buildGoogleMapsUrl(r.route.depot, r.route.stops, r.route.returnToDepot), '_blank')
  }

  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-50 dark:bg-slate-900">
      {/* Header */}
      <div className="flex items-center justify-between gap-3 px-4 sm:px-6 h-14 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shrink-0">
      ...
      {/* Toolbar */}
      <div className="px-4 sm:px-6 py-3 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shrink-0 space-y-3">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-4 text-sm tabular-nums">
            <span className="text-slate-500 dark:text-slate-400">{t('route.stops')}: <span className="font-semibold text-slate-900 dark:text-white">{r.selectedCount}/{r.candidateCount}</span></span>
            <span className="text-slate-500 dark:text-slate-400">{t('route.items')}: <span className="font-semibold text-slate-900 dark:text-white">{Math.round(r.itemCount)}</span></span>
            {r.route && <span className="text-slate-500 dark:text-slate-400">{t('route.distance')}: <span className="font-semibold text-slate-900 dark:text-white">{formatDistance(r.route.totals.distanceMeters)}</span></span>}
            {r.route && <span className="text-slate-500 dark:text-slate-400">{formatDuration(r.route.totals.durationSeconds)}</span>}
          </div>
          {r.dirty && r.route && (
        ...
        <div className="flex items-center gap-4 flex-wrap text-sm">
          <label className="flex items-center gap-2 text-slate-600 dark:text-slate-400">
            ...
            <input type="time" value={r.departureHHmm} onChange={e => r.updateDeparture(e.target.value)}
              className="px-2 py-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-cyan-500" />
          </label>
        ...
        {/* segmented view toggle (only once a route exists) */}
        {r.route && (
          <div className="inline-flex rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-hidden">
            {(['delivery', 'loading'] as const).map((v, i) => (
              <button key={v} onClick={() => setView(v)}
                className={`px-3 py-1.5 text-sm font-medium transition-colors ${view === v ? 'bg-cyan-600 text-white' : 'text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'} ${i > 0 ? 'border-l border-slate-200 dark:border-slate-700' : ''}`}>
      ...
      <div className="flex-1 overflow-auto px-4 sm:px-6 py-4">
        {r.loadingCandidates ? (
          <div className="flex items-center justify-center h-64"><Loader2 className="w-8 h-8 text-cyan-600 animate-spin" /></div>
        ) : r.candidatesError ? (
      ...
      {/* Sticky action bar */}
      <div className="px-4 sm:px-6 py-3 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 shrink-0">
        <div className="max-w-3xl mx-auto flex items-center gap-2">
          ...
          <button onClick={openInMaps} disabled={!canExport}
            className="inline-flex items-center justify-center gap-2 px-3 py-2.5 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 text-sm font-medium rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors disabled:opacity-50" title={t('route.openInGoogleMaps')}>
            <Navigation className="w-4 h-4" /> <span className="hidden md:inline">{t('route.openInGoogleMaps')}</span>
          </button>
